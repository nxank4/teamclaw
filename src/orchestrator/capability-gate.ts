/**
 * Runtime capability gate for orchestrated agents.
 *
 * Every tool call flows through {@link gateToolCall} before reaching
 * the executor. The gate is the only enforcement boundary for tool
 * access — agent prompts can lie, the gate cannot.
 *
 * Two checks, in order:
 *   1. Tool allowlist: tool_name must appear in agent_def.tools.
 *   2. Write-scope glob: for tools in {@link WRITE_TOOLS} only, when
 *      write_scope is set, tool_args.path must match at least one glob.
 *
 * Globs are evaluated by minimatch with `dot: true` so paths under
 * `__tests__/` and other dotfile directories match.
 *
 * The subagent runner surfaces denials back to the LLM as tool results
 * so the agent can recover (try a different file, give up) rather than
 * crashing the turn.
 */
...
export function gateToolCall(input: GateInput): GateDecision {
  ...
  if (!agent_def.tools.includes(tool_name)) {
    return {
      allowed: false,
      agent_id,
      tool: tool_name,
