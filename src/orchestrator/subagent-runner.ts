 ...
 *
 * Every agent invocation flows through {@link runSubagent}. There is no
 * direct path from the orchestrator to the LLM — that is what guarantees
 * the depth limit and the capability gate hold uniformly.
 *
 ...
 *   - File-lock acquisition on write tools before the underlying tool
 *     runs; `releaseAllFor(agent_id)` at turn end drains every lock held
 *     by this agent, even on thrown errors.
...
import { type AgentDefinition, WRITE_TOOLS } from "./types.js";
import { WriteLockManager, WriteLockTimeoutError } from "./write-lock.js";

/**
 * Opaque artifact-store handle. The runner never reads it; legacy
 * callers (crew-runner, compaction) pass a real ArtifactStoreReader
 * here for ABI continuity. New callers may pass anything (or omit).
 */
export type ArtifactStoreReader = unknown;
 ...
 *   - tokens_breakdown: opt-in input/output split for UI/profiling
 */
 ...
 * (when supplied) and into the `subagent_returned` debug log entry.
 */
...
export interface RunSubagentArgs {
  ...
  prompt: string;
  /** Opaque — preserved for ABI compatibility with legacy crew callers. The runner does not read it. */
  ...
  /** Observability sink for tool-call lifecycle events. */
  onProgress?: SubagentProgressEmitter;
  /** Per-token streaming sink, forwarded straight into {@link runAgentTurn}. */
...
function buildSystemPrompt(args: RunSubagentArgs): string {
  ...
  const scopeLine = agent_def.write_scope?.length
    ? `Your write/edit calls are restricted to paths matching:\n${agent_def.write_scope.map((g) => `  - ${g}`).join("\n")}\n`
    : "";
 ...
 * parsing the returned `summary`, persisting any output, and merging
...
export async function runSubagent(
  ...
  if (depth > MAX_SUBAGENT_DEPTH) {
    debugLog("warn", "orchestrator", "subagent_depth_exceeded", {
      data: {
  ...
  if (inputEstimate > token_budget.max_input) {
    debugLog("warn", "orchestrator", "subagent_budget_exceeded", {
      data: {
        agent_id: agent_def.id,
  ...
  debugLog("info", "orchestrator", "subagent_spawned", {
    data: {
      agent_id: agent_def.id,
      parent_agent_id,
      depth,
  ...
  const preToolHook = (name: string, toolArgs: Record<string, unknown>): string | null => {
    ...
    debugLog("info", "orchestrator", "tool_forbidden", {
      data: {
  ...
  // onProgress channel. The mapping narrows the status to the
  // SubagentProgressEvent union so callers get a typed payload.
  ...
  try {
    result = await runAgentTurnImpl({
      ...
      maxTurns,
      signal,
      source: `orchestrator:subagent:${agent_def.id}`,
    });
  } finally {
    const released = write_lock_manager.releaseAllFor(agent_def.id);
    if (released.length > 0) {
      debugLog("debug", "orchestrator", "subagent_locks_released", {
  ...
  debugLog("info", "orchestrator", "subagent_returned", {
    data: {
      agent_id: agent_def.id,
      depth,
      tokens_used,
