/**
 * WriteLockManager — session-scoped, reentrant write locks for orchestrated agents.
 *
 * Lock keys are conventionally namespaced:
 *   - `file:<absPath>`        guards a single file from concurrent writers
 *   - `artifact:<sessionId>`  serializes per-session writes
 *
 * Semantics:
 ...
 *
 * The subagent runner is responsible for releasing all locks held by an
 * agent at turn end.
 */
...
export class WriteLockManager {
  ...
  acquire(
    ...
    if (!state) {
      ...
      debugLog("info", "orchestrator", "write_lock_acquired", {
    ...
    return new Promise<void>((resolve, reject) => {
      const timer = setTimeout(() => {
        ...
        debugLog("warn", "orchestrator", "write_lock_timeout", {
          data: {
            key,
            agent_id: agentId,
            holder: state.holder,
      ...
      debugLog("debug", "orchestrator", "write_lock_queued", {
        data: {
          key,
          agent_id: agentId,
          holder: state.holder,
  ...
  tryAcquire(key: string, agentId: string): WriteLockResult {
    ...
    debugLog("info", "orchestrator", "write_lock_denied", {
      data: {
        key,
        agent_id: agentId,
        holder: state.holder,
  ...
  release(key: string, agentId: string): void {
    ...
    debugLog("info", "orchestrator", "write_lock_released", {
