...
export async function renderAuditPDF(
  ...
  return new Promise((resolve, reject) => {
    const doc = new PDFDocument({
      ...
      info: {
        Title: `TeamClaw Audit Trail — ${audit.sessionId}`,
        Author: "TeamClaw",
        Subject: audit.goal,
      },
    });
    ...
    // Header
    doc.fontSize(20).font("Helvetica-Bold").text("TeamClaw Audit Trail");
    doc.moveDown(0.5);
    ...
    doc.fontSize(8).font("Helvetica")
      .text(`Generated by TeamClaw | Session: ${audit.sessionId}`, { align: "center" });
