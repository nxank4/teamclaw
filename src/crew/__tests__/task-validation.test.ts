...
type RunnerInternals = {
  taskDidWrite(task: CrewTask): boolean;
  taskExpectsWrite(task: CrewTask): boolean;
  missingDescribedFiles(task: CrewTask): string[];
  state: { tasks: CrewTask[]; currentTaskIndex: number };
};

function internals(r: CrewRunner): RunnerInternals {
  return r as unknown as RunnerInternals;
}

describe("CrewRunner validation leniency fix", () => {
  ...
  it("taskExpectsWrite returns false for 'configure' and 'setup' alone (dropped from keywords)", () => {
    const runner = new CrewRunner(mockRegistry());
    const t1: CrewTask = { id: "a", description: "Configure eslint rules", status: "in_progress" };
    const t2: CrewTask = { id: "b", description: "Setup the CI pipeline", status: "in_progress" };
    expect(internals(runner).taskExpectsWrite(t1)).toBe(false);
    expect(internals(runner).taskExpectsWrite(t2)).toBe(false);
  });
});

describe("CrewRunner missingDescribedFiles", () => {
  let tmpDir: string;
  ...
  it("returns empty when description mentions no file paths", () => {
    const runner = new CrewRunner(mockRegistry());
    const task: CrewTask = { id: "t", description: "Do something abstract", status: "in_progress" };
...
describe("CrewRunner validation helpers", () => {
...
describe("CrewRunner.recordToolCall — parallel attribution", () => {
  it("attributes to the explicit taskIndex, not state.currentTaskIndex", () => {
    const runner = new CrewRunner(mockRegistry());
    // Populate state with two in-progress tasks
    const taskA: CrewTask = { id: "a", description: "create A", status: "in_progress" };
    const taskB: CrewTask = { id: "b", description: "create B", status: "in_progress" };
  ...
  it("falls back to currentTaskIndex when taskIndex is omitted", () => {
    const runner = new CrewRunner(mockRegistry());
    const taskA: CrewTask = { id: "a", description: "x", status: "in_progress" };
    const taskB: CrewTask = { id: "b", description: "y", status: "in_progress" };
    internals(runner).state.tasks = [taskA, taskB];
    internals(runner).state.currentTaskIndex = 1;
  ...
  it("preserves exitCode and stderrHead in toolCallResults", () => {
    const runner = new CrewRunner(mockRegistry());
    const task: CrewTask = { id: "t", description: "x", status: "in_progress" };
  ...
  it("no-ops when the addressed task is not in_progress", () => {
    const runner = new CrewRunner(mockRegistry());
    const task: CrewTask = { id: "t", description: "x", status: "completed" };
    internals(runner).state.tasks = [task];
